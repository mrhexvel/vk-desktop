...
export async function fetchUserInfo(accessToken: string, userId: number): Promise<VKUserResponse> {
  try {
    const response = await fetch(
      `https://api.vk.com/method/users.get?user_ids=${userId}&fields=photo_200&access_token=${accessToken}&v=5.199`,
    )
    const data = (await response.json()) as { response?: VKUserResponse[]; error?: { error_msg: string } }
...
export async function checkAccessPermissions(accessToken: string): Promise<string[]> {
  try {
    const response = await fetch(
      `https://api.vk.com/method/account.getAppPermissions?access_token=${accessToken}&v=5.199`,
    )
    const data = (await response.json()) as { response?: number; error?: { error_msg: string } }
...
export async function getStickerInfo(
  ...
  try {
    ...
    const data = (await response.json()) as { response?: VKStickerInfo; error?: { error_msg: string } }
